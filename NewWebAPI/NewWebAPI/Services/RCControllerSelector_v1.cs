...
{
    public class RCControllerSelector : DefaultHttpControllerSelector
    {
        private HttpConfiguration _config;

        public RCControllerSelector(HttpConfiguration config): base(config)
        {
            _config = config;
        }
        ...
        private string GetVersionFromMediaType(HttpRequestMessage request)
        {
            ...
            var ex = new Regex(@"application\/vnd\.nwa\.([a-z]+)\.v([0-9]+)\+json", RegexOptions.IgnoreCase);
        ...
        {
            const string HEADER_NAME = "X-NewWebAPI-Version";
