create procedure [dbo].[sp_ifc_LoadCommitments_Product]
@Iteration			int,
...
as
	set nocount on;
begin
	set @CalculateAmounts = isnull(@CalculateAmounts,'N');
	/* ======================================================================================================= 
	 ================================== FIRST RUN  ============================================================ 
	 ========================================================================================================== */
	if @Iteration = 1
	begin
		if @CalculateAmounts = 'N'
		begin
		...
		begin
			...
			from	#lcsfMasterLoad a
					inner join @Keys b				
						...
						and a.DealID		= b.DealID;
			---------------------------------------------------------------------------------			
			exec dbo.sp_ifc_LoadCommitments_UpdateSizes @Iteration = 1, @CallType = 1;
		end
	end		
	/* ======================================================================================================== 
	 ================================== NEXT RUN ============================================================= 
	...
	begin
		...
		begin
			...
			where	a.Iteration = @PrvIteration;
			---------------------------------------------------------------------------------			
			exec dbo.sp_ifc_LoadCommitments_UpdateSizes @Iteration = @Iteration, @CallType = 1;
		end	
	end		
/* ========================================================================================================
 ================================== RECURSIVE CALL ======================================================== 
...
GO
grant execute on dbo.sp_ifc_LoadCommitments_Product to PE_datawriter, PE_datareader
GO