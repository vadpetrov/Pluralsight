create procedure [dbo].[sp_ifc_LoadCommitments_SPV_Adapter]
...
/* ==================================================================================================================
	...
	create unique index LCSPV_FEEDER_PERCENT_IPVD_UIDX on #tmpFeederPercent (pei_invstr_id_c,pei_invst_fnd_id_c,pei_vhcl_id_c,pei_deal_id_c);	
================================================================================================================== */
	...
	---------------------------------------
	if object_id('tempdb..#tmpFeederPercent') is not null	
		and
		not exists(
		select	1 
		from	tempdb.sys.indexes 
		where	object_id = object_id('tempdb..#tmpFeederPercent')
				and name = 'LCSPV_FEEDER_PERCENT_IPVD_UIDX')
	begin	
		create unique index LCSPV_FEEDER_PERCENT_IPVD_UIDX on #tmpFeederPercent (pei_invstr_id_c,pei_invst_fnd_id_c,pei_vhcl_id_c,pei_deal_id_c);
	end	
/* ================================================================================================================== */
...
/* ============================================================================================================= */
	if @LoadDetails = 'Y'
	begin	
		with ipvd_total(InvestorID,ProductID,VehicleID,DealID,MltTotalAmount) as
		(
			select	a.ReportInvestorID,
					a.ReportProductID,
					a.ReportVehicleID,
					a.ReportDealID,
					cast(max(a.MltPVDTotalAdj) as float)
			from 	#lcsfMasterData a			
			group by a.ReportInvestorID, a.ReportProductID,a.ReportVehicleID,a.ReportDealID
		),	
		ipvd_total_run(InvestorID,ProductID,VehicleID,DealID,MltTotalAmount) as
		(
		...
		from	ipvd_total_run a
				inner join 	ipvd_total b				
...
GO
grant execute on dbo.sp_ifc_LoadCommitments_SPV_Adapter to PE_datawriter, PE_datareader
GO