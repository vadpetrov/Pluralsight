create procedure [dbo].[sp_ifc_LoadCommitments_SPV_Adapter_Investor]
...
/* ==================================================================================================================
	...
	(		
		...
		InvestorAllcPct			float
	);
	create unique index LCSPV_ALLC_FEEDER_DET_ADAPTER_IPVDFPFVFI_UIDX on #tmpFeederDtl(pei_invstr_id_c,pei_invst_fnd_id_c,pei_vhcl_id_c,pei_deal_id_c,FeederFundID,FeederVhclID,FeederInvstrID);	
================================================================================================================== */
	...
	if object_id('tempdb..#tmpFeederDtl') is not null	
		...
		where	object_id = object_id('tempdb..#tmpFeederDtl')
				and name = 'LCSPV_ALLC_FEEDER_DET_ADAPTER_IPVDFPFVFI_UIDX')
	begin	
		create unique index LCSPV_ALLC_FEEDER_DET_ADAPTER_IPVDFPFVFI_UIDX on #tmpFeederDtl(pei_invstr_id_c,pei_invst_fnd_id_c,pei_vhcl_id_c,pei_deal_id_c,FeederFundID,FeederVhclID,FeederInvestorID);
	end	
/* ================================================================================================================== */
	...
	create index LCSF_MASTER_ADAPTER_INVESTORALLCPCT_ROWID_IDX on #lcsfAdapterIAP (RowID);
	create index LCSF_MASTER_ADAPTER_INVESTORALLCPCT_IPVD_IDX on #lcsfAdapterIAP (InvestorID,ProductID,VehicleID,DealID,FeederProductID,FeederVehicleID,FeederInvestorID);
	------------------------------------------------------------
...
/* ============================================================================================================= */	
	exec dbo.sp_ifc_LoadCommitments
	@ProductID 			= @ProductID,
	@DealID 			= @DealID,
...
/* ============================================================================================================= */	
	...
	select	a.ReportDealID,
			...
			case a.Iteration when 1 then null else a.RunInvestorID end
...
/* ============================================================================================================= */
	...
	begin	
		...
		pvd_total_run(InvestorID,ProductID,VehicleID,DealID,TotalAmount,MltTotalAmount,
						IsFeeder,HasFeeder,FeederFundID,FeederVhclID,FeederInvestorID) as
		(
			select	a.pei_invstr_id_c,
					...
					a.FeederInvestorID
			from 	#tmpAlloc_Feeder a
			group by a.pei_invstr_id_c,a.pei_invst_fnd_id_c,a.pei_vhcl_id_c,a.pei_deal_id_c,
					a.FeederFundID,a.FeederVhclID,a.FeederInvestorID
		)
	...
	--					and name = 'LCSPV_ALLC_FEEDER_DET_ADAPTER_IPVDFPFVFI_UIDX')
	--begin
	--	drop index LCSPV_ALLC_FEEDER_DET_ADAPTER_IPVDFPFVFI_UIDX on #tmpFeederDtl;
	--end							
	-------------------------------------------------------------------
	...
	drop table #lcsfAdapterIAP;
end

GO
grant execute on dbo.sp_ifc_LoadCommitments_SPV_Adapter_Investor to PE_datawriter, PE_datareader
GO